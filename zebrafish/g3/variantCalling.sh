...
# Define file paths
reference_genome="/data/okendojo/zebrafish/refGenome/GRCz11_genomic.fasta"  # Replace with the path to your reference genome
output_directory="gatk_variant_output"    # Directory to store output files
#BAM="/data/okendojo/zebrafish/data/g3/rna_sequences/gene_quantification/rnaseq_result/star_salmon/merged.bam"
...
#samtools merge -O BAM -@24 -o $output_directory/aligned_sorted.bam "/data/okendojo/zebrafish/data/g3/eQTL/BAMs/*.bam"

##bamtools merge -list /data/okendojo/zebrafish/data/g3/eQTL/BAMs/bam.list -out $output_directory/aligned_sorted.bam 

#Add readgroups and index bam file

#java -Xmx32g -XX:ParallelGCThreads=10 -jar $PICARDJARPATH/picard.jar AddOrReplaceReadGroups -I $output_directory/aligned_sorted.bam  -O $output_directory/aligned_sorted_RG.bam -RGID 4 -RGLB lib1 -RGPL ILLUMINA -RGPU unit1 -RGSM 20 -SO coordinate --CREATE_INDEX true

# Step 4: Mark Duplicates with Picard
#java -Xmx32g -XX:ParallelGCThreads=10 -jar $PICARDJARPATH/picard.jar MarkDuplicates -I $output_directory/aligned_sorted_RG.bam -O $output_directory/aligned_marked.bam -M $output_directory/marked_dup_metrics.txt
...
gatk HaplotypeCaller -R "$reference_genome" -I "$output_directory/aligned_sorted_RG.bam" -O "$output_directory/raw_variants.vcf"
