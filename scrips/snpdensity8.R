...
# Label only concordant genes in the top quartile of score
true_scores <- concordance_df[consistent_direction == TRUE & is.finite(score), score]
score_threshold <- if (length(true_scores) > 0) {
  as.numeric(quantile(true_scores, 0.75, na.rm = TRUE))
} else {
  Inf
}

label_df <- concordance_df[
  consistent_direction == TRUE &
    is.finite(score) &
    score >= score_threshold,
  .(gene_id, delta_male_female_nadia, delta_male_female_cb, score)
]
...
annot_source <- stats[order(-score)][1:min(.N, 10), .(
  gene_id,
  mid_kb,
  delta_male_female_nadia,
  delta_male_female_cb
...
# ============================================================
# Main and supplement figures
# ============================================================
main_figure <- p1 / p2 / p3_haps +
  plot_layout(heights = c(1.1, 1.1, 1.4))

supplement_figure <- p3 / p4_zero / p5_concord +
  plot_layout(heights = c(1.0, 0.9, 1.0))

ggsave(
  filename = file.path(outdir, "plots", "main_publication_figure.pdf"),
  plot = main_figure,
  width = 14,
  height = 18
)
ggsave(
  filename = file.path(outdir, "plots", "main_publication_figure.png"),
  plot = main_figure,
  width = 14,
  height = 18,
  dpi = 300
...
ggsave(
  filename = file.path(outdir, "plots", "supplement_figure.pdf"),
  plot = supplement_figure,
  width = 14,
  height = 18
)
ggsave(
  filename = file.path(outdir, "plots", "supplement_figure.png"),
  plot = supplement_figure,
  width = 14,
  height = 18,
  dpi = 300
