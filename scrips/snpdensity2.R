...
# ============================================================
# Fixed metadata (19 rows, no recycling)
# ============================================================
meta_txt <- "
...
bed[, `:=`(
  start = as.integer(start0) + 1L,   # convert BED to 1-based inclusive
  end = as.integer(end),
...
# Reorder metadata to match VCF sample order
...
# ============================================================
# Map variants to BED intervals
# Use non-equi join instead of foverlaps()
# ============================================================
hits <- bed[variants,
...
# Variant if genotype is not homozygous reference and not missing
...
# 2) Male-female delta plot, with separate annotations for each comparison
delta_df <- stats %>%
...
# 4) Heatmap of top candidate intervals
...
ggsave(file.path(outdir, "plots", "sex_linked_snp_density_panels.pdf"),
       plot = combined, width = 14, height = 18)
ggsave(file.path(outdir, "plots", "sex_linked_snp_density_panels.png"),
       plot = combined, width = 14, height = 18, dpi = 300)

ggsave(file.path(outdir, "plots", "male_female_density_track.pdf"),
       plot = p1, width = 14, height = 8)
ggsave(file.path(outdir, "plots", "male_female_density_track.png"),
       plot = p1, width = 14, height = 8, dpi = 300)

ggsave(file.path(outdir, "plots", "male_female_delta_track.pdf"),
       plot = p2, width = 14, height = 8)
ggsave(file.path(outdir, "plots", "male_female_delta_track.png"),
       plot = p2, width = 14, height = 8, dpi = 300)

ggsave(file.path(outdir, "plots", "nadia_cb_scatter.pdf"),
       plot = p3, width = 12, height = 6)
ggsave(file.path(outdir, "plots", "nadia_cb_scatter.png"),
       plot = p3, width = 12, height = 6, dpi = 300)

ggsave(file.path(outdir, "plots", "top_interval_heatmap.pdf"),
       plot = p4, width = 12, height = 10)
ggsave(file.path(outdir, "plots", "top_interval_heatmap.png"),
