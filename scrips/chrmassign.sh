cat /data/okendojo/zebrafish/data/fish6/asm/variationAnalysis/danRer4.fasta | awk -v SEQ="" '{if (match($1, ">") && $1 != NAME) { if (SEQ != "") { print NAME; print toupper(SEQ); } SEQ=""; NAME=$1; } else {SEQ=SEQ""$1;} } END { print NAME; print toupper(SEQ)}' | sed -r s/'A{1,}+'/A/g |sed -r s/'C{1,}+'/C/g |sed -r s/'G{1,}+'/G/g |sed -r s/'T{1,}+'/T/g > compressed.fasta


if [ ! -e mashmap.out ]; then
   mashmap -r /data/okendojo/zebrafish/data/fish6/asm/variationAnalysis/danRer4.fasta -q assembly.fasta --pi 95 -s 10000 -t 8
fi
...
if [ ! -e compressed.mashmap.out ]; then
   ...
   mashmap -r /data/okendojo/zebrafish/data/fish6/asm/asm_verkko22/compressed.fasta -q assembly.homopolymer-compressed.fasta --pi 95 -s 10000 -t 8
   cd ..
