...
# Inputs
# ============================================================
bed_file <- "/data/okendojo/zebrafish/data/g3/sex_project/asm/t2tMap/bam_fixed_rg/genes_on_NC_133179.1.bed"
...
# ============================================================
sex_cols <- c(
  Female = "#D81B60",  # strong magenta
  Male   = "#1E88E5"   # strong blue
)

comp_cols <- c(
  "Nadia: Male - Female" = "#E69F00",  # orange
  "CB: Male - Female"    = "#0072B2"   # blue
)
...
# 2) Sex-associated SNP density differences with strong comparison colors
delta_df <- stats %>%
...
# 3) Sex density comparison by WT line with distinctive male/female colors
p3 <- ggplot(
  line_sex_summary,
  aes(x = sex, y = mean_density, fill = sex, color = sex)
) +
  geom_boxplot(width = 0.55, alpha = 0.20, outlier.shape = NA, linewidth = 0.4) +
  geom_jitter(width = 0.14, height = 0, alpha = 0.75, size = 1.4) +
  facet_wrap(~ WT_line, ncol = 2) +
  ...
  labs(
    ...
    fill = "Sex",
    color = "Sex",
    title = "Sex density comparison by WT line"
  ) +
  theme_bw(base_size = 11) +
  theme(
    legend.position = "top",
...
ggsave(
  filename = file.path(outdir, "plots", "sex_density_by_wt_line.pdf"),
  plot = p3, width = 12, height = 6
)
ggsave(
  filename = file.path(outdir, "plots", "sex_density_by_wt_line.png"),
