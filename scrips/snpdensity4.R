...
# ============================================================
# Count SNPs per sample per interval
# ============================================================
...
# 3) Publication-quality scatter plot: female vs male SNP density
scatter_df <- data.table(
  interval_key = stats$interval_key,
  gene_id = stats$gene_id,
  mid_kb = stats$mid_kb,
  Nadia_female = stats$mean_female_nadia,
  Nadia_male = stats$mean_male_nadia,
  CB_female = stats$mean_female_cb,
  CB_male = stats$mean_male_cb
)

scatter_long <- rbindlist(list(
  scatter_df[, .(
    WT_line = "Nadia",
    female_density = Nadia_female,
    male_density = Nadia_male,
    gene_id,
    interval_key,
    mid_kb
  )],
  scatter_df[, .(
    WT_line = "CB",
    female_density = CB_female,
    male_density = CB_male,
    gene_id,
    interval_key,
    mid_kb
  )]
...
p3 <- ggplot(scatter_long, aes(x = female_density, y = male_density, color = WT_line)) +
...
combined <- p1 / p2 / p3 / p4 + plot_layout(heights = c(1.2, 1.2, 1.35, 1.2))
