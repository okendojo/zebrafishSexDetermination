...
safe_spearman <- function(x, y) {
  ...
  if (sum(keep) < 3) return(list(rho = NA_real_, p.value = NA_real_))
  out <- tryCatch(cor.test(x[keep], y[keep], method = "spearman"), error = function(e) NULL)
  if (is.null(out)) return(list(rho = NA_real_, p.value = NA_real_))
  list(rho = unname(out$estimate), p.value = out$p.value)
}
...
stats_list <- lapply(split(sample_density, by = "interval_idx"), function(d) {
  ...
  d[1, .(
    ...
    zero_prop_all = mean(snp_density == 0),
...
# 1) Zero-density summary by group
zero_density_summary <- sample_density[, .(
  n_intervals = .N,
  zero_intervals = sum(snp_density == 0),
  zero_prop = mean(snp_density == 0),
  mean_density = mean(snp_density)
), by = .(WT_line, sex, haps)]
fwrite(zero_density_summary, file.path(outdir, "zero_density_summary.tsv"), sep = "\t")

# 2) Concordance between Nadia and CB sex effects
concordance_df <- stats[, .(
  interval_key, gene_id, chrom, start, end, mid_kb,
...
fwrite(concordance_df, file.path(outdir, "interval_concordance.tsv"), sep = "\t")

# 3) Ranked candidate intervals
top_candidates <- stats[sex_linked_flag == TRUE][order(-score)]
fwrite(top_candidates, file.path(outdir, "top_candidate_intervals.tsv"), sep = "\t")

# 4) Correlation of female vs male densities within each WT line
line_correlations <- rbindlist(lapply(c("Nadia", "CB"), function(line) {
  if (line == "Nadia") {
    f <- stats$mean_female_nadia
    m <- stats$mean_male_nadia
  } else {
    f <- stats$mean_female_cb
    m <- stats$mean_male_cb
  }
  ct <- safe_spearman(f, m)
  data.table(
    WT_line = line,
    spearman_rho = ct$rho,
    p_value = ct$p.value,
    n_intervals = sum(is.finite(f) & is.finite(m))
  )
}))
fwrite(line_correlations, file.path(outdir, "line_correlation_summary.tsv"), sep = "\t")
...
# Label data for top candidate intervals (data-driven, not target-specific)
label_df <- stats[order(-score)][1:min(.N, 10), .(
  gene_id,
  mid_kb,
  delta_male_female_nadia,
  delta_male_female_cb
...
# 4) Haplotype-specific density line plot with 95% CI
p3_haps <- ggplot(
  hap_line_summary,
...
# 5) Zero-density by group
p4_zero <- ggplot(zero_density_summary, aes(x = haps, y = zero_prop, fill = sex)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  facet_wrap(~ WT_line) +
  scale_fill_manual(values = sex_cols) +
  labs(
    x = "Haplotype",
    y = "Proportion of zero-density intervals",
    fill = "Sex",
    title = "Zero-SNP intervals by haplotype and sex"
  ) +
  theme_bw(base_size = 11) +
  theme(
    legend.position = "top",
...
# 6) Concordance of Nadia vs CB sex-effect direction
p5_concord <- ggplot(concordance_df, aes(x = delta_male_female_nadia, y = delta_male_female_cb, color = sex_linked_flag)) +
  geom_hline(yintercept = 0, linetype = 2, linewidth = 0.4, color = "grey50") +
  geom_vline(xintercept = 0, linetype = 2, linewidth = 0.4, color = "grey50") +
  geom_point(size = 2, alpha = 0.85) +
  geom_abline(slope = 1, intercept = 0, linetype = 3, linewidth = 0.5, color = "black") +
  scale_color_manual(values = c(`TRUE` = "#D81B60", `FALSE` = "grey40")) +
  labs(
    x = "Nadia: Male - Female density difference",
    y = "CB: Male - Female density difference",
    color = "Concordant",
    title = "Concordance of sex effects between Nadia and CB"
  ) +
  theme_classic(base_size = 11) +
  theme(
    legend.position = "top",
  ...
  geom_text_repel(
    data = stats[order(-score)][1:min(.N, 10), .(
      gene_id,
      delta_male_female_nadia,
      delta_male_female_cb
    )],
    aes(x = delta_male_female_nadia, y = delta_male_female_cb, label = gene_id),
    inherit.aes = FALSE,
    size = 3,
    label.size = 0.25,
    fill = "white",
...
combined <- p1 / p2 / p3 / p3_haps / p4_zero / p5_concord +
  plot_layout(heights = c(1.1, 1.1, 1.1, 1.4, 0.9, 1.0))

ggsave(
  filename = file.path(outdir, "plots", "sex_linked_snp_density_panels.pdf"),
  plot = combined, width = 14, height = 26
)
ggsave(
  filename = file.path(outdir, "plots", "sex_linked_snp_density_panels.png"),
  plot = combined, width = 14, height = 26, dpi = 300
)
...
ggsave(
  filename = file.path(outdir, "plots", "zero_density_by_group.pdf"),
  plot = p4_zero, width = 12.5, height = 6
)
ggsave(
  filename = file.path(outdir, "plots", "zero_density_by_group.png"),
  plot = p4_zero, width = 12.5, height = 6, dpi = 300
)

ggsave(
  filename = file.path(outdir, "plots", "concordance_nadia_cb.pdf"),
  plot = p5_concord, width = 12.5, height = 6
)
ggsave(
  filename = file.path(outdir, "plots", "concordance_nadia_cb.png"),
  plot = p5_concord, width = 12.5, height = 6, dpi = 300
)
